...
class Portfolio4(Portfolio):
    ...
    def backtest_portfolio(self):
        ...
        for i in range(positions_matrix.shape[0]):
            ...
            positions_matrix[i, :6] = start_capital[i] * self.signals.iloc[i,0]/len(stock_subclass)
            positions_matrix[i, 6:] = start_capital[i] * self.signals.iloc[i,1]/len(bond_subclass)
...
if __name__ == "__main__":
    ...
    StrategyFour = Strategy4(df, df2)
    signals = StrategyFour.generate_signals()

    PortfolioFour = Portfolio4(class_returns, subclass_returns, stock_subclass, bond_subclass, signals, initial_capital)
    portfolio4, position4, cache4 = PortfolioFour.backtest_portfolio()  
