    ...
    def backtest_portfolio(self):
        ...
        for i in range(positions_matrix.shape[0]):
            if self.signals.iloc[i,0] == 1:
                positions_matrix[i,:6] = capital/len(self.stock_subclass)
            else:
                positions_matrix[i, 6:] = capital/len(self.bond_subclass)
...
if __name__ == "__main__":
    ...
    StrategyOne = Strategy1(df, df2)
    signals = StrategyOne.generate_signals()
    
    PortfolioOne = Portfolio1(class_returns, subclass_returns, stock_subclass, bond_subclass, signals, initial_capital)
    
    portfolio1, cache1 = PortfolioOne.backtest_portfolio()  
