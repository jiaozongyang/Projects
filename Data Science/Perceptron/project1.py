...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    z = label * (np.matmul(feature_vector,theta) + theta_0)
    if z >= 1:
        return 0
    else:
        return 1-z
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    z = labels * (np.matmul(feature_matrix,theta)+theta_0)
    loss = np.empty(len(z))
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
            y_pred = np.matmul(feature_matrix[i],current_theta) + current_theta_0
            if labels[i] * y_pred  <= 0:
                current_theta = current_theta + labels[i] * feature_matrix[i]
...
def average_perceptron(feature_matrix, labels, T):
    ...
    current_theta = np.zeros(feature_matrix.shape[1],)
    current_theta_0 = 0
    thetas = np.zeros(feature_matrix.shape[1],)
    theta_0s = []
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            theta_0s.append(current_theta_0)

    return (thetas/(T*feature_matrix.shape[0]),sum(theta_0s)/len(theta_0s))
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    res = np.empty(feature_matrix.shape[0])
    y_pred = np.matmul(feature_matrix,theta)+theta_0
...
def bag_of_words(texts):
    ...
    for text in texts:
        ...
        for word in word_list:
            if (word not in dictionary) & (word not in stop):
